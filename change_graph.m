...
%     'String','Surf','Position',[315,135,70,25],...
...
hgo = uicontrol('Style','pushbutton',... 
    'String','Go','Position',[315,135,70,25],...
...
align([hgo,htext,hpopup],'Center','None'); %hsurf,hmesh,hcontour

% Initialize the GUI. 
% Change units to normalized so components resize automatically. 
set([f,hgo,htext,hpopup],'Units','normalized'); %hsurf,hmesh,hcontour,

% Generate the data to plot. 
peaks_data = peaks(35); 
membrane_data = membrane; [x,y] = meshgrid(-8:.5:8); 
r = sqrt(x.^2+y.^2) + eps; 
sinc_data = sin(r)./r;
...
% end
function gobutton_Callback(source,eventdata) % Display contour plot of the currently selected data. 
    surf(current_data); 
end
